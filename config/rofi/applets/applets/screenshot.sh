...
case $chosen in
  $screen)
    if [[ -f /usr/bin/scrot ]]; then
      sleep 1; scrot 'Screenshot_%Y-%m-%d-%S_$wx$h.png' -e "mv \$f ${target}/\$f"
    else
      msg
    fi
    ;;
  $area)
    if [[ -f /usr/bin/scrot ]]; then
      scrot -s 'Screenshot_%Y-%m-%d-%S_$wx$h.png' -e "mv \$f ${target}/\$f"
    else
      msg
    fi
    ;;
  $window)
    if [[ -f /usr/bin/scrot ]]; then
      sleep 1; scrot -u 'Screenshot_%Y-%m-%d-%S_$wx$h.png' -e "mv \$f ${target}/\$f"
    else
      msg
    fi
    ;;
