...
local function set_wallpaper(s)
    -- Wallpaper
    beautiful.wallpaper = home .. "/Imágenes/Nordic-theme.jpg"
...
local startsh = home .. "/.config/awesome/autostart.sh"
local restartsh = home .. "/.config/awesome/.aw-restart"

local function startup(s)
    --local st = string.format("bash -c \"rm %s || %s\"", restartsh, startsh)
    local st = home .. startsh
    awful.spawn(st)
end
...
--[[
   ...
   [    'exit',
   [    function (args)
   [        awful.spawn(string.format("touch %s", restartsh))
   [    end
   [)
   [
   [awesome.connect_signal(
...
globalkeys = gears.table.join(
    ...
    -- Volume
    awful.key({}, "XF86AudioRaiseVolume", function() awful.spawn(home .. "/.config/awesome/scripts/qvol.sh up") end,
        {description = "raise volume", group = "client"}),

    awful.key({}, "XF86AudioLowerVolume", function() awful.spawn(home .. "/.config/awesome/scripts/qvol.sh down") end,
        {description = "lower volume", group = "client"}),

    awful.key({}, "XF86AudioMute", function() awful.spawn(home .. "/.config/awesome/scripts/qvol.sh mute") end,
